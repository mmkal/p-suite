...
const getPackageDetails = (modulePath, packageName) => {
	let cwd = path.dirname(modulePath);
	while (true) {
		...
		if (result.packageJson.name === packageName) {
			return result;
		}

		const parentDirectory = path.dirname(path.dirname(result.path));
		if (parentDirectory === path.dirname(result.path)) {
			return result;
		}
...
const getPackageDescription = ({ packageJson, readmeContent }) => {
...
const generate = async () => {
	...
	const beginGeneratedPackageDocsMarker =
	...
	if (beginIndex === -1 || endIndex === -1) {
		throw new Error(
			`Add ${beginGeneratedPackageDocsMarker} and ${endGeneratedPackageDocsMarker} to the readme somewhere.`,
		);
	}
			...
			[
				...
				s.packages
					.map((p) => {
						...
						return `- [**${p.name}**](#${p.name}): ${details.description}`;
					})
					.join("\n"),
			].join("\n\n"),
		),
	...
	newGeneratedReadmeSection = newGeneratedReadmeSection.replaceAll(
	...
	await execa("pnpm", [
		"install",
		...packages.map((p) => `${p.package}@latest`),
	]);
	...
	for (const p of packages) {
		...
		fs.writeFileSync(
			path.join(sourceDirectory, `${p.package}.js`),
			code.join("\n"),
		);
		fs.writeFileSync(
			path.join(sourceDirectory, `${p.package}.d.ts`),
			code.join("\n"),
		);
	...
	packages.forEach((p) => {
		...
		Object.entries(engines).forEach(([tool, versionString]) => {
			const parts = versionString.split(" || ").flatMap((versionString) => {
				const joined = [packageJson.engines[tool], versionString]
					.filter(Boolean)
					.join(" ");
